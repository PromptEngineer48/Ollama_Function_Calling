...
def get_bible_verse(book: str, chapter: int, verse: int) -> Optional[Dict[str, Any]]:
    """
    Fetch a specific verse from a given book and chapter of the Holy Bible using the RapidAPI Holy Bible API.
    """
    ...
    try:
        ...
        # Debug: Print the raw response to investigate issues
        # print("Raw API Response:", response.json())
        
        data = response.json()

        if "Book" in data and "Chapter" in data and "Verse" in data and "Output" in data:
            return data
        else:
            print(f"Verse not found for {book} {chapter}:{verse}.")
            return None
        ...
        if response.message.tool_calls:
            for tool in response.message.tool_calls:
                if function_to_call := available_functions.get(tool.function.name):
                    ...
                    elif function_to_call == get_bible_verse:
                        results2 = function_to_call(**tool.function.arguments)
                        results3 = f"{results2['Book']} {results2['Chapter']}:{results2['Verse']} - {results2['Output']}"
                        print("Bible", results3)
